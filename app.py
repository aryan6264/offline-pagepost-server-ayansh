...
message_count = 0  # How many comments sent
is_commenting = False  # Is bot running?
...
def send_comments(access_tokens, post_id, prefix, time_interval, messages):
    global message_count, is_commenting
    is_commenting = True
...
def send_comment():
    ...
    <body>
      ...
      <div class="container text-center">
        ...
        <form method="post" action="/stop/{session_id}">
...
def stop_sending_by_id(sid):
    if sid == session_id:
        stop_event.set()
        return f'✅ Session {sid} stopped successfully.'
    else:
        return f'❌ Invalid Session ID: {sid}', 400

@app.route('/status', methods=['GET'])
def get_status():
    return {
        "status": "✅ Commenting Active" if is_commenting else "❌ Not Active",
